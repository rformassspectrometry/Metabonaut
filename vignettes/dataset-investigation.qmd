...
cardiovascular disease (CVD) and healthy individuals (CTR). The subset analyzed
...
serum samples from a different cohort, while the study samples represent blood
plasma samples, from a different sample collection.
...
Below we load the `lcms1` object that we saved after preprocessing (described in
the *Complete end-to-end LC-MS/MS Metabolomic Data Analysis* workflow).
...
an interesting technical evaluation of the data. In our case we expect very
...
Although there seem to be differences in background noise between CVD and CTR,
these are only very small.

The code above calculated background signal as the difference between the total
sum of signal and the quantified abundance of the LC-MS features. An
alternative, and more accurate but more computationally intense, approach to
estimate the background signal, which removes actual mass peaks for identified
chromatographic peaks from the full MS data is performed in the next section.
...
of the first sample. We further expand the *m/z* range by 3ppm on each side
since the reported `"mzmin"` and `"mzmax"` values from the *centWave* peak
detection seemed to underestimate the actual *m/z* peak width
slightly. Increasing *m/z* peak width by such a small value is not problematic
for centroided data but it ensures the estimated *background* signal to not
contain any *foreground* peak signals.

Using the `keep` parameter of the `filterPeaksRanges()` function it is possible
to define whether the mass peaks matching the specified filters are kept or
removed. We can thus keep or remove all mass peaks of the identified
chromatographic peaks to define the foreground or background signal,
respectively.
...
the mass peaks **outside** of the chromatographic peaks. To illustrate this we
plot below first the full MS data for a retention time and *m/z* range
containing also the signal of a methionine ion. The lower panel of the plot
contains the individual mass peaks in the region with their intensity
color-coded. The area of identified chromatographic peaks is indicated with red
rectangles. The upper panel shows the TIC signal of the data shown in the lower
panel.

```{r}
#| fig-cap: "Figure 11. Full MS data for a retention time region rfom 145-175 seconds and an m/z range from 150-160."
...
```{r}
#| fig-cap: "Figure 12. Mass peaks within chromatographic peak areas."
...
```{r}
#| fig-cap: "Figure 13. Mass peaks outside chromatographic peak areas."
filterSpectra(one_bg, filterRt, c(145, 175)) |>
    filterSpectra(filterMzRange, c(150, 160)) |>
    plot()
```

These plots show the effect of filtering the data set performed above - the
foreground-only data contains all mass peaks within the chromatographic peak
areas, while the background data contains (mostly) low abundant (background)
signal, or signal that was not defined as, or included in, chromatographic
peaks.
...
```{r}
#| fig-cap: "Figure 14. Comparison of total ion signal, total foreground signal and total background signal."
