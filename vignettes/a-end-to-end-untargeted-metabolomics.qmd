...
intern_standard <- read.delim("vignettes/intern_standard_list.txt")
...
fData(eic_is)[
  c("cystine_13C_15N", "methionine_13C_15N"),
  c("name", "mz", "rt")
] |>
  kable(format = "pipe")
```
...
### 1. Extracting MS2 Spectra for Features

The first step is to identify which MS2 spectra belong to the defined chromatographic
features. We define the retention time and m/z ranges for all features and then
extract the corresponding MS2 scans.
...
### 2. Combining Spectra into Consensus Spectra

Often, a single feature will trigger multiple MS2 scans. To obtain a high-quality spectrum for annotation, we merge these multiple scans into a single "consensus" spectrum per feature. In this step, we keep only those peaks that are present in at least 75% of the scans for a given feature to reduce noise.
...
### 3. Exporting Data for GNPS

Finally, we export the data in the specific formats required by GNPS. This involves creating a feature quantification table (.txt) and a spectral data file (.mgf). Note that we filter the feature table to ensure it strictly matches the features present in the MGF file.
