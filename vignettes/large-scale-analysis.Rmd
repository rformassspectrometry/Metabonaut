...
subtitle: "Supplementary Information for: *xcms in peak form: Now anchoring a complete
...
This document describes the preprocessing of a large public untargeted
metabolomics dataset using the *[xcms](https://bioconductor.org/packages/3.22/xcms)* package using its new
memory-saving data representation mode. This data representation mode enables
analysis of very large data sets (the present consisting of about 4,000 samples)
also on conventional computer hardware. Performance and memory usage of
different storage modes are subsequently evaluated on a smaller data
subset. Finally, details on the internal data and memory handling of *xcms* are
presented and properties of different configurations for efficient (and
parallel) processing of large scale data discussed.
...
version >= 3.8 which available since Bioconductor release 3.22.
...
data in files ending with *01.CDF* and the respective MS2 scans in a file with
...
package and Bioconductor's *[MsBackendMetaboLights](https://bioconductor.org/packages/3.22/MsBackendMetaboLights)* packages are
...
tbl <- data.frame(
    ...
    check.names = FALSE)
pandoc.table(
    tbl, style = "rmarkdown", split.table = Inf,
...
![Base peak chromatogram of the full data set.](large-scale-bpc-1.png)
...
![EIC plots for 3 compounds in 200 randomly selected samples.](large-scale-eic-random-1.png)
...
![EIC plots for 3 compounds in 200 randomly selected samples with identified chromatographic peaks highlighted (grey color).](large-scale-eic-random-cp-1.png)
...
![EIC plots for 3 compounds in 5 samples. Detected chromatographic peaks are indicated with a grey color.](large-scale-eic-random-cp-5-1.png)
...
![Example EICs after alignment.](large-scale-eic-random-align-1.png)

The EICs are nicely aligned (see Figure 5).
...
![Evaluation of correspondence settings on compound 1.](large-scale-feature-1-1.png)
...
![Evaluation of correspondence settings on compound 2.](large-scale-feature-2-1.png)
...
![Evaluation of correspondence settings on compound 3.](large-scale-feature-3-1.png)
...
`Spectra` backend from the *[MsBackendSql](https://bioconductor.org/packages/3.22/MsBackendSql)* package (see the last
...
#> 1 tmp<-peaksData(spectra(tsub[1:8]))             6.45             1922.9
#>   Peak_RAM_Used_MiB
#> 1            3865.1
```
...
|:------------------:|:----:|:--------------:|:---------------------:|
|   XcmsExperiment   |  1   |      5984      |         38.34         |
|   XcmsExperiment   |  2   |      5920      |         24.74         |
|   XcmsExperiment   |  4   |      5920      |         18.21         |
|   XcmsExperiment   |  8   |      5920      |         16.18         |
| XcmsExperimentHdf5 |  1   |      5920      |         38.39         |
| XcmsExperimentHdf5 |  2   |      5920      |         24.91         |
| XcmsExperimentHdf5 |  4   |      5920      |         18.25         |
| XcmsExperimentHdf5 |  8   |      5920      |         16.6          |
...
``` r
plot(tbl[1:4, "CPUs"], tbl[1:4, "Processing time [min]"],
     type = "b", col = "#ff000080", xlab = "CPUs", ylab = "Time [min]")
points(tbl[5:8, "CPUs"], tbl[5:8, "Processing time [min]"],
       type = "b", col = "#0000ff80")
grid()
...
![Comparison of computation time depending on the number of CPUs employed for the 2 different xcms result storage modes.](large-scale-performance-1.png)
...
|   Result object    | CPUs | Peak RAM [MiB] | Processing time [min] |
|:------------------:|:----:|:--------------:|:---------------------:|
|   XcmsExperiment   |  1   |      5920      |         6.712         |
|   XcmsExperiment   |  2   |      5863      |         5.611         |
|   XcmsExperiment   |  4   |      5863      |         5.007         |
|   XcmsExperiment   |  8   |      5864      |         5.673         |
| XcmsExperimentHdf5 |  1   |      5864      |         6.979         |
| XcmsExperimentHdf5 |  2   |      5859      |         5.752         |
| XcmsExperimentHdf5 |  4   |      5859      |         5.19          |
| XcmsExperimentHdf5 |  8   |      5859      |         7.397         |
...
|:------------------:|:----:|:--------------:|:---------------------:|
|   XcmsExperiment   |  1   |      4166      |         27.49         |
| XcmsExperimentHdf5 |  1   |      3970      |         27.44         |
...
|   Result object    | CPUs | Peak RAM [MiB] | Processing time [min] |
|:------------------:|:----:|:--------------:|:---------------------:|
|   XcmsExperiment   |  1   |      5859      |         5.942         |
|   XcmsExperiment   |  2   |      5834      |         2.839         |
|   XcmsExperiment   |  4   |      5834      |         2.839         |
|   XcmsExperiment   |  8   |      5834      |         2.952         |
| XcmsExperimentHdf5 |  1   |      5834      |         6.127         |
| XcmsExperimentHdf5 |  2   |      5834      |         5.309         |
| XcmsExperimentHdf5 |  4   |      5834      |         4.847         |
| XcmsExperimentHdf5 |  8   |      5834      |         5.395         |
...
realized in memory. By using the *[Spectra](https://bioconductor.org/packages/3.22/Spectra)* package to represent and
...
- For the present analysis, a big part of computation time was spent on the
  ...
  *[MsbackendSql](https://bioconductor.org/packages/3.22/MsbackendSql)* for details. The code that could be used to store
...
#>  [3] peakRAM_1.0.2               xcms_4.8.0
#>  [5] MsIO_0.0.12                 MsExperiment_1.12.0
#>  [7] ProtGenerics_1.42.0         MsBackendMetaboLights_1.4.2
...
#>   [5] clue_0.3-66                 MassSpecWavelet_1.76.0
...
#>  [31] Rhdf5lib_1.32.0             parallel_4.5.2
#>  [33] prettyunits_1.2.0           cluster_2.1.8.1
#>  [35] R6_2.6.1                    stringi_1.8.7
#>  [37] RColorBrewer_1.1-3          limma_3.66.0
#>  [39] GenomicRanges_1.62.0        iterators_1.0.14
#>  [41] Rcpp_1.1.0                  Seqinfo_1.0.0
#>  [43] SummarizedExperiment_1.40.0 IRanges_2.44.0
...
#>  [53] affy_1.88.0                 codetools_0.2-20
#>  [55] curl_7.0.0                  lattice_0.22-7
#>  [57] tibble_3.3.0                plyr_1.8.9
#>  [59] withr_3.0.2                 Biobase_2.70.0
#>  [61] S7_0.2.0                    benchmarkmeData_1.0.4
#>  [63] evaluate_1.0.5              BiocFileCache_3.0.0
#>  [65] alabaster.schemas_1.10.0    affyio_1.80.0
#>  [67] pillar_1.11.1               BiocManager_1.30.27
#>  [69] filelock_1.0.3              MatrixGenerics_1.22.0
#>  [71] foreach_1.5.2               MALDIquant_1.22.3
#>  [73] MSnbase_2.36.0              ncdf4_1.24
#>  [75] hms_1.1.4                   ggplot2_4.0.1
#>  [77] scales_1.4.0                alabaster.base_1.10.0
#>  [79] glue_1.8.0                  MsFeatures_1.18.0
#>  [81] lazyeval_0.2.2              tools_4.5.2
#>  [83] mzID_1.48.0                 data.table_1.17.8
#>  [85] vsn_3.78.0                  QFeatures_1.20.0
#>  [87] mzR_2.44.0                  XML_3.99-0.20
#>  [89] fs_1.6.6                    rhdf5_2.54.0
#>  [91] grid_4.5.2                  impute_1.84.0
#>  [93] tidyr_1.3.1                 MsCoreUtils_1.21.0
...
#> [103] digest_0.6.38               SparseArray_1.10.1
