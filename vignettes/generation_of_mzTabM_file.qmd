---
title: "Generation of mzTab-M file"
...
vignette: >
  %\VignetteIndexEntry{Generation of mzTab-M file}
...
end-to-end metabolomics data workflow in mzTab-M format, based on the
Metabolights data set *MTBLS8735*. The original *xcms*
preprocessing result object is available in Metabonaut and is included also
within the *RmzTabM* package. After collecting all necessary experimental
metadata, we export this result object as a mzTab-M file with only the metadata
...
of the data and follow FAIR principles. The mzTab-M has stringent requirement
in mandatory fields and their format. Refer to the official documentation for
additional information. We need to add/adapt some columns of the
`SummarizedExperiment`'s `colData()` to complete or match the expected format:
