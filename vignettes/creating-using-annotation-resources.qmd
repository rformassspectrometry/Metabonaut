...
resources (DOI: 10.5281/zenodo.17232042) to a local cache folder.

::: {.callout-note}
This vignette downloads spectral libraries from Zenodo and MassBank, which can
take several minutes depending on your internet connection. Downloaded files are
cached in a local directory to avoid repeated downloads across renders.
:::
...
pth <- file.path("objects", "annotation_cache")
dir.create(pth, showWarnings = FALSE, recursive = TRUE)

mgf_fl <- file.path(pth,
                    "GNPS_Drug_Library_Spectra_Drugs_and_Metabolites.mgf")
if (!file.exists(mgf_fl)) {
...
pth_mb <- file.path("objects", "massbank_cache")
dir.create(pth_mb, showWarnings = FALSE, recursive = TRUE)

#' Only download if not already cached; retry up to 3 times
if (length(dir(pth_mb)) == 0L) {
    for (.attempt in seq_len(3)) {
        ok <- tryCatch({
            download_zenodo(doi, path = pth_mb, quiet = TRUE, timeout = 600)
            TRUE
        }, error = function(e) FALSE)
        if (ok) break
        Sys.sleep(10)
...
has_mb <- length(dir(pth_mb, pattern = "\\.(zip|txt)$")) > 0L
dir(pth_mb)
```

The archive contains several thousands of *.txt* files in MassBank format, one
for each spectrum. We next unzip the archive to the same cache folder and
get the listing of all data files.

```{r}
#| eval: !expr has_mb
#' unzip the archive
unzip(file.path(pth_mb, dir(pth_mb, pattern = "\\.zip$")[1L]), exdir = pth_mb)

#' get the file listing
dr <- dir(pth_mb, pattern = "MassBank-MassBank", full.names = TRUE)
