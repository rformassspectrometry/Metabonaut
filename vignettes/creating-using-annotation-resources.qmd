...
`Spectra` object using the `r Biocpkg("MsBackendMgf")`, which supports import
of all metadata fields, renaming and mapping them to specific spectra variables. Below
...
Compound annotations are sparse. Spectrum names hold the compound, adduct, and
sometimes collision energy, but formatting is inconsistent:
...
Two things stand out: intensities are *absolute*, and low-abundance peaks likely
reflect instrument noise. We remove peaks below 1% of the maximum and scale
intensities to sum to 1. This is one possible cleaning choice, not a standard.
...
with *matchms* and access it in R through `MsBackendPy` (`r Biocpkg("SpectriPy")`)
[@graeve_spectripy_2025].
...
The `metaBlocks` parameter can import extra metadata. MassBank metadata are
*generally* more standardized and include more compound information 
(e.g., formula, exact mass). We now match the end-to-end vignette spectra 
against MassBank 2025.10 with `matchSpectra()`.
...
Alternatively, store annotations in SQLite via `r Biocpkg("CompoundDb")`, which
provides a simple schema for small compounds and MS data. Here we build a
database from the GNPS2 drug library; see the vignette
[*Creating CompoundDb annotation resources*](https://rformassspectrometry.github.io/CompoundDb/articles/create-compounddb.html)
for more examples.
...
`precursor_mz`. `CompoundDb` accepts additional columns. We extract these from
the GNPS2 `Spectra` object into a `data.frame`.
...
We link spectra to compounds by adding unique IDs for each table.
...
inally we define database metadata (origin, version, DOI) for reproducibility.
...
For direct use, load the database as a `CompDb` annotation resource:
...
This file is manageable; the full GNPS library is not.
...
extension of in-house libraries (see the *CompoundDb* vignette mentionned above).
